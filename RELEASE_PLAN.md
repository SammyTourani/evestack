...
### Real Mac workflow checkpoint — September 16

- The operator authorized a clean installation and real local-model workflow on the 8 GB Mac. A fresh candidate install ran Eve 0.54.5, native PostgreSQL 17.10 with pgvector 0.8.6, and a virtual filesystem containing copied real repository files. Docker and hosted account authorization were not used.
- This supersedes the preceding local-model, memory and clock verification gaps: actual file reads, a browser task, remember/recall, an inspected correction with a new vector and unchanged owner, restart persistence, audited decisions, one scheduled occurrence, local webhook receipt, three-turn CLI/replay and observed cancellation passed. [Full evidence and boundaries](MAC_SMOKE_TEST.md).
- The workflow exposed the unsupported status enum comparison and obsolete continuation-token commands. Fixes also make replay advance by turn ID and preserve independently pending decisions. Tests now use the pinned world's real migrations and Eve's actual request parser.
- Llama 1B and non-thinking Qwen 0.6B failed meaningful tool-use attempts. Thinking-enabled Qwen completed the narrow workflows but repeated a pending deletion request; the approval gate held. Local-model guidance now states those limits and distinguishes download size from loaded memory.
- The new source checkpoint requires fresh CI and image checks. External account/channel setup, uncoached first use and public-registry/image installation after publication remain outstanding.
