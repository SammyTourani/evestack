...
export default {
  ...
  async available() {
    ...
    // Postgres, because the classification half builds a fixture: with no
    // provider key every turn finishes instantly, so the sweep correctly has
    // nothing to classify and those assertions would run over an empty list.
  ...
  async run(t) {
     ...
     *
     * ── This was removed once, and is back on purpose ────────────────────────
     *
     * The first time it ran, the sweep answered `unknown` carrying "the agent
     * could not be reached (terminated)" about an agent this probe had just
     * talked to twice. Nobody could say whether that was a real defect or an
     * artefact of the fixture, so it came out rather than be asserted on a
     * guess.
     *
     * It is back because the mechanism was found: readRecentEvents threw when
     * eve advertised a tail index higher than the number of lines it then wrote
     * AND dropped the socket instead of ending the response — one shape out of
     * eight, narrowed against a stub and now fixed and pinned by
     * packages/dashboard/test/agent-stream.test.mjs. lib/fleet.ts caught that
     * throw and reported it as an unreachable agent, which is how a healthy
     * network got the blame.
     *
     * So this is no longer asserting a guess: it is the check that says the fix
