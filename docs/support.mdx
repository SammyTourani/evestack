---
title: Support
description: Which versions get fixes and what the Linux, macOS and Windows checks actually cover.
---
...
A non-blocking `windows-latest` CI job runs Node 24 workspace builds/typechecks and the
CLI/scaffolder platform suites. It includes ASCII output, path handling and native Windows
ACL checks for private configuration and backups. Inspect the job's conclusion directly:
`continue-on-error` means a failed platform job can coexist with an overall passing workflow.

The interactive wizard, browser-opening helpers and full Docker runtime are not covered by
that job. Linux has the broader runtime coverage. WSL2 is an option when you need the Linux
execution path, but it does not replace testing your own Windows installation.
...
- **Postgres** is tested as `pgvector/pgvector:pg17` — the image both compose files run, and the
  ...
  `vector(n)` column.
- **Browsers.** The dashboard is a Next app and has no browser test matrix. Website CI uses Chromium. Dashboard release checks use a separate local Chromium fixture
  for task, decision, routine and recovery flows. Safari and Firefox are not in the browser matrix.
...
3. **Assumptions about eve are checked by machine, not by memory.** `contract/` checks runtime and repository assumptions
   and goes red when eve's behaviour drifts under them, and `eve-watch` runs them against every
