...
| Retention | durable session state; use a reviewed pruning policy | exported trace rows can be pruned under a retention policy; historical evidence may be lost |
...
these table definitions. The `evestack` schema also stores remembered facts,
reviews, routines, budget settings and regression cases. **Do not drop the whole
schema to repair or downgrade the trace tier.** Run the matching dashboard image,
back up Postgres, and use a component-specific recovery or retention procedure.
Exported traces may no longer be available from the agent after its own retention
window, so do not assume deleted trace evidence can always be reconstructed.
