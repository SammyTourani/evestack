...
- **With several sinks configured, all of them must accept before a transition counts as
  delivered.** Marking it done because one worked would mean the other never receives that alert,
  and the reason to configure two is that you do not trust one.
...
| Table | What it holds |
| --- | --- |
| `evestack.alert_state` | one row per monitor: what was last **seen**, and what was last **delivered** |
