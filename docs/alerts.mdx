...
- **Two dashboards will not double-send.** A short lease row in `evestack.alert_lease` gives one
  instance the tick; it expires rather than being released, so a crash mid-send is picked up by the
  other one interval later.
