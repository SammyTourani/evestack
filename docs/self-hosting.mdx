...
| --- | --- | --- | --- |
| Agent, `eve dev` | 2000 | **`127.0.0.1`** | Does **not** auto-increment in a scaffolded project — a busy port is a hard `EADDRINUSE` |
...
<Warning>
  ...
  2026-08-09: `{ beta: '5.0.0-beta.32', latest: '4.3.3' }` — `latest` is a whole major behind the
  protocol eve speaks, and that gap has held for as long as this file has existed. eve 0.30.8
  builds against `@workflow/core@5.0.0-beta.39`, and the runtime rejects an incompatible protocol
  version outright — the failure is at boot, which is the good case. The beta tag moves often
  (it was `beta.31` four days ago), so check it yourself rather than trusting this line:
...
```bash
docker build -t ghcr.io/sammytourani/evestack-dashboard:0.3.1 -f packages/dashboard/Dockerfile .
```
