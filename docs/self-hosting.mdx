...
| --- | --- | --- | --- |
| Agent, `eve dev` | 2000 | all interfaces | A bare `eve dev` auto-increments if 2000 is taken. A scaffolded project does not: `npm run dev` passes `--port`, so a busy port is `EADDRINUSE` |
