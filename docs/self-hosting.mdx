...
still grants inside `EVE_DEV=1` — and it goes red against 0.29.5. The measured result for
every published eve in the supported range, and for the three below it, is in
[Upgrading eve](/docs/upgrading#what-has-actually-been-run).
...
The template ships `networkPolicy: "deny-all"`, not eve's `allow-all`. The session container is
attached to no network at all, so anything the model's shell tries to fetch — `curl`,
`git clone`, `pip install`, `npm install`, `apt-get install` — fails on DNS. Loopback still
works, and model calls, Postgres, Composio and the channels are all outside the sandbox and
unaffected. Two variables move it, both read at container-creation time:

| Variable | Default | Effect |
| --- | --- | --- |
| `EVESTACK_SANDBOX_NETWORK` | `deny-all` | `allow-all` gives the shell the host's network. Any other value is a boot error. |
| `EVESTACK_SANDBOX_IMAGE` | the digest pinned in `sandbox.ts` | Overrides the sandbox runtime image. |

eve keeps one container per durable session and reattaches to it across restarts without
re-applying the policy, so a change to either variable reaches new sessions only. See
[Troubleshooting](/docs/troubleshooting) for the failure text and for how to drop the old
containers.
...
<Note>
  ...
  ~204 MB compressed per platform. `.github/workflows/publish-dashboard.yml` builds each arch on
  its own native runner and merges the digests, so neither is emulated.
</Note>
