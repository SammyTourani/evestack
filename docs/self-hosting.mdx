...
[Compatibility](/docs/compatibility) for which eve versions have been run against this suite.
...
There is **no published image**. Build one from a checkout — the context is the repository
**root**, not `packages/dashboard`:

```bash
docker build -t evestack-dashboard:local -f packages/dashboard/Dockerfile .
```
...
From this repository the whole thing is one command, which builds that image for you:

```bash
docker compose --profile full up -d
```

Publishing an image later is a one-step follow-up: `docker build` as above, tag it for a
registry, `docker push`. The compose service that `create-evestack` generates already reads
`${EVESTACK_DASHBOARD_IMAGE:-evestack-dashboard:local}`, so a published tag becomes a default
...
<CardGroup cols={2}>
  <Card title="Compatibility" icon="check" href="/docs/compatibility">
    Which eve versions this stack has actually been run against, and what the contract suite pins.
  </Card>
