...
The dashboard binds to `127.0.0.1` in `docker-compose.yml`, deliberately. It is not a viewer: it
can start sessions, send follow-ups, resolve pending tool approvals, and cancel runs. It also
ships no identity provider of its own, on purpose — it reads the approver's identity from the
request (`EVESTACK_APPROVER_HEADER`, `X-Forwarded-User`, `X-Forwarded-Email`, or the Basic user)
and records how that identity was established, so it is designed to sit behind something you
already trust: a reverse proxy doing OAuth, Cloudflare Access, a VPN. Publishing port 4000 without
one of those in front publishes an unauthenticated button that can approve a shell command.

If the dashboard is not on the same host as the agent, give it `EVESTACK_AGENT_URL` plus the
agent's own `EVESTACK_AUTH_USER` / `EVESTACK_AUTH_PASSWORD` — it attaches Basic credentials only
when both are set. Its one required variable remains `WORKFLOW_POSTGRES_URL`; everything on the
observe side is a SQL read. See [The dashboard](/docs/dashboard).
