...
<Warning>
  **Both cells in that first row were wrong until 2026-08-09, and the binding one matters on
  Linux.** They said "all interfaces" and "auto-increments if 2000 is taken". Neither is true of
  a scaffolded project:

  - `eve dev` falls back to `DEFAULT_DEVELOPMENT_SERVER_HOST`, which is `127.0.0.1`
    (`dist/src/internal/nitro/host/dev-server-url.js`). Loopback, not all interfaces.
  - `retryOnAddressInUse` is set only when **no** port is passed, and `scripts/dev.mjs` always
    passes `--port` from `EVESTACK_AGENT_PORT`. So a collision throws rather than moving.
    (`eve dev` with no port *does* auto-increment — which is why the template's own comment
    describing that behaviour is correct and this table was not.)
...
<Steps>
  <Step title="Pin the three versions that have to agree">
    ...
    of the three is checked for you. The shipped `package.json` pins `eve` at `^0.54.3` and
    `@workflow/world-postgres` at exactly `5.0.0-beta.42` — an exact version, not a range and not
    a dist-tag. `>=0.30.0` is the floor for any deployment reachable from a network — see
    [Auth off Vercel](#auth-off-vercel).
  </Step>
  ...
  <Step title="Build and start">
    ...
    `--port`, which beats `$PORT` and eve's own default of 3000. That is the number `npm run
    verify` probes and the number the generated compose file points the dashboard at, so there is
    ...
    [Operations](/docs/operations#run-the-agent-as-a-service), which also covers why the agent is
...
<Callout title="Upgrading an existing database from eve 0.30 to 0.54">
  The World spec moves from **5** to **7**, and the spec-7 World keeps a per-run slot sequencer
  in a table the older schema does not have. Booting does **not** create it. Measured on a
  database holding 3 runs and 52 spec-5 events:

  - the upgraded agent **starts** against the old database, and old runs stay readable — a
    request against a finished one answers `session_not_active`, not a parse error;
  - creating a **new** session fails with
    `insert into "workflow"."workflow_event_slots" … Queue execution failed (500)`;
  - `npm run db:bootstrap` creates the missing table, keeps every existing row, and new sessions
    then work. Afterwards both generations coexist: spec 5 with 52 events, spec 7 alongside it.

  So the upgrade is two steps, not one: install, then **re-run `npm run db:bootstrap`** before
  the first boot. Nothing is lost, but skipping the second step leaves an agent that reads
  history and cannot start a conversation.
</Callout>


<Warning>
  Pin `@workflow/world-postgres` to an **exact version** — `5.0.0-beta.42`, which is what
  `templates/default` declares. Never `latest`, and never the `beta` dist-tag.
  ...
  incompatible worlds on different days. The World a given release declares comes from the exact
  `@workflow/world` it pins, and eve embeds the one spec it will accept:

  | `world-postgres` | pulls `@workflow/world` | World spec | eve 0.30.8 | eve 0.54.3 |
  | --- | --- | --- | --- | --- |
  | `5.0.0-beta.32` | `5.0.0-beta.25` | 5 | boots | `worker init failed` |
  | `5.0.0-beta.42` | `5.0.0-beta.35` | 7 | `worker init failed` | boots |

  This is not theoretical. Every published `create-evestack` from `0.3.1` to `0.10.0` shipped the
  `beta` tag; when it moved from `.32` to `.42` the World's spec went 5 → 7 while the template
  still pinned eve 0.30.8, and every new project died on its first `npm run dev` with
  `requires a World with matching spec version 5, but the configured World declares spec
  version 7`. Nothing in the repository had changed. `^5.0.0-beta.42` and `~5.0.0-beta.42` are
  not fixes either — both float forward inside the same prerelease line. Move the pin only after
  installing a candidate and watching it boot.

  `packages/create-evestack/test/no-floating-tags.test.mjs` now fails the build if any dependency
  in the scaffold is specified by tag rather than by version.
</Warning>
...
SECURITY.md and every published peer range state; the template pins `^0.54.3` because that is
...
session starts, returns `202` with a session ID, and then stalls forever because the run's
...
<Note>
  Published since `dashboard-v0.1.0` as a multi-arch manifest — `linux/amd64` and `linux/arm64`,
...
that nothing prunes for you — have a page of their own: [Operations](/docs/operations).
...
The agent has no such condition because it is not a compose service. Under a unit it exits fast
and the restart policy retries, which is the same guarantee reached differently; see
[Operations](/docs/operations#run-the-agent-as-a-service) for why the restart limiter has to be
...
| `evestack` | evestack | Memories/vectors, approvals, reviews/audit, spans/facts, routines/runs/notifications, budgets and regression cases |
...
agent's `evestack.memories` table is created lazily on the first `remember` call.
If it already contains data, the archive must restore that data and its vector
index. Recreating an empty table later does not recover old memories.
...
measured. [Operations](/docs/operations#retention-the-workflow-schema-is-never-pruned) has the
opt-in pruning procedure and, more importantly, what is safe to delete and what eve still needs.

<Note>
  Follow [Backup and restore](/docs/backup-restore) to rehearse the archive in an
  isolated database. Keep it disconnected from agents and schedulers while
  inspecting it: restored jobs can resume external actions on startup.
</Note>
...
<CardGroup cols={2}>
  <Card title="Operations" icon="gear" href="/docs/operations">
    Supervising the agent, bounding the logs, and pruning a schema nothing prunes for you.
  </Card>
