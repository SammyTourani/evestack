---
title: CLI
description: Create and operate an Eve Stack project, inspect work and safely preview configuration changes.
---

`evestack` provides these project commands:
...
evestack routines             inspect recurring work and its run history
...
evestack configure            preview, back up and save supported configuration
evestack upgrade              compare with this CLI's bundled template
...
## Preview and restore configuration

```bash
evestack configure --set=EVESTACK_PROVIDER=openai --set=EVESTACK_MODEL=YOUR_MODEL
evestack configure --from-file=/private/settings.json
```

Put secrets in a private JSON file, not command-line arguments. Values are strings;
JSON `null` removes an override. The command previews supported settings with
secret values redacted and prints a fingerprint. Repeat the same options with
`--apply --expect=FINGERPRINT` to save that exact preview. Stale files, unsafe Git
tracking, invalid env syntax and unsupported values are refused.

Apply takes an owner-protected backup outside the project before atomically
replacing configuration. Use `--restore=BACKUP_ID` to preview a restore, then apply
its new fingerprint. A restore takes another backup. Backups retain old secrets
and cover env configuration only; they do not back up Postgres.

Saving does not activate a provider or channel. Restart the affected processes,
run `evestack verify`, then inspect a real task or receipt. For supported heartbeat
settings, use the template's `npm run heartbeat:preview` before enabling. The
preview shows the exact prompt and quiet-hour gate without sending anything.

## Upgrade preview

`evestack upgrade --changed-only` compares your code and declared dependencies
with this CLI's bundled template. `--json` includes hashes and component versions.
It writes nothing and has no apply flag. Differences require manual merging;
symlinks and oversized files are flagged without reading their targets. Secrets,
caches and extra user files are excluded. Follow the [upgrade procedure](/docs/upgrading)
for backups, publication checks and activation.
...
<Note>
  `evestack open` is this command's first name and still works. It was renamed because
  `dashboard` is the word someone reaches for when they want the dashboard and have not read
  anything — and there are now four other ways to the same page, so the name had to be the
  guessable one. See [The dashboard](/docs/dashboard#five-ways-in).
</Note>
...
Writes the setup pack bundled with this CLI release: `SKILL.md` plus four reference files.
The website's **Set up your agent** button follows the deployed site version and may differ.
Full detail in [Set up with an agent](/docs/agent-setup).
...
The default works offline after CLI installation and reports its source and version.
`EVESTACK_PACK_URL` explicitly chooses a custom server, which is marked version-unverified.
Remote HTTPS (or loopback HTTP) is required, with no redirects, a 15-second timeout and a
1 MiB response limit. Existing files require `--force`; linked destination files/directories
are refused. If a filesystem write fails partway through, inspect the destination before retrying.

| Code | Meaning |
| --- | --- |
| `0` | Installed, or printed. |
| `1` | Could not load the pack, or refused to write. |
| `2` | Bad arguments. |
...
The preview names detected compatibility, files, ports and trace-export changes.
Trace export can include prompts and tool inputs; decide whether that destination
is appropriate before applying. Existing instrumentation is preserved.

After applying, start the services using the printed commands, then run
`evestack status` and `evestack readiness` against that dashboard. Readiness
labels unavailable and unverified checks; follow [the first-task guide](/docs/first-task)
to verify a real model/source result. `attach` writing files does not establish
that the services are running. Keep its undo instructions and the private
configuration backup when [removing the attachment](/docs/uninstall).
