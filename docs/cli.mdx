...
Writes the evestack skill pack — `SKILL.md` plus four reference files — where your coding agent
will find it, so it knows this project without being told. The same pack the landing page's
**Set up your agent** button copies. Full detail in [Set up with an agent](/docs/agent-setup).
...
The pack is fetched from the site rather than bundled into this package, so it cannot ship stale —
which is the trade: it needs a network connection, and says so plainly when it does not have one.
