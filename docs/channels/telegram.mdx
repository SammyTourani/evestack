...
eve 0.30.2 implements only the second one. We checked the shipped adapter rather than guessing:
...
Make sure the agent is up (`pnpm dev` in `templates/default`, serving `:2000`), then DM your bot
...
That is exactly the key [`@evestack/budget`](/registry) caps spend against, so a per-principal
