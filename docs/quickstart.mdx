---
title: Quickstart
description: Create the workspace, start services and verify your first model connection.
---
...
- An `OPENAI_API_KEY` or an `ANTHROPIC_API_KEY`, or a supported provider configured in
  [local setup](/docs/local-setup). [Ollama](/docs/local-setup#ollama) runs models locally
  when your machine has enough memory; hardware and electricity still cost money.

After setup, follow [Your first useful task](/docs/first-task): connect a repository,
review a maintenance brief and test it as a routine before enabling repeats.
...
`verify`, `attach`, `doctor` — see [the CLI reference](/docs/cli)), and `create-evestack` is
the name npm's `create-*` convention leads people to. Same code, same prompts, same flags.

It asks **four** questions, all of them before any work starts, so the install and the image
pull are a wait you can walk away from:
...
4. **Bring it up** — whether it should start Postgres, create the schema and pull the
   dashboard for you.
...
<Warning>
  ...
  forward is enough to kill a boot. The scaffolded `package.json` pins `5.0.0-beta.42`; leave it
...
The agent boots on the port the scaffolder wrote into `.env.local` as `EVESTACK_AGENT_PORT` —
...
Keep the `sessionId` returned by this request. Stop the dev server (`Ctrl-C`),
restart it (`npm run dev`), and send a follow-up to that same session:

```bash
curl -X POST http://127.0.0.1:2000/eve/v1/session/SESSION_ID \
  -H 'content-type: application/json' \
  -d '{"message":"What exact phrase did I ask you to remember?"}'
```

Replace `SESSION_ID` with the saved value and inspect the reply and memory tool
results. Current Eve routes use the session ID directly; do not send a
`continuationToken`.
