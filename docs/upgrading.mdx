...
directory once. `evestack upgrade` now provides a read-only comparison against the
template bundled with that CLI version. It does not fetch a newer release,
overwrite files, install dependencies, restart services or migrate a database.
...
From inside your project, using the CLI version you want to evaluate:

```bash
evestack upgrade --changed-only
evestack upgrade --json
```

The preview lists declared dependency changes and file hashes. `same` means the
bytes match; `missing` is a candidate addition; `review` requires a manual merge;
`blocked` means a file could not be safely inspected. Symlinks, non-regular files
and oversized files are not followed or guessed. No contents of `.env`,
`.env.local`, `.npmrc` or caches are read. Extra project files are not proposed
for deletion.

An older project has no original template baseline, so differences cannot be
automatically attributed to your changes versus old defaults. Retain customized
agent setup, dependencies, project names and Compose choices. Use the printed
candidate directory for local file comparison; the command offers no apply flag.

New scaffolds carry `evestack-release.json`: the selected component versions,
runtime requirements and versioned storage guards. The source repository's
`release-manifest.json` is regenerated from those definitions and checked by CI.
A recorded manifest is a reference, not proof of installed dependencies, running
services or publication. Check the registry and deployment before updating it.

Use [Backup and restore](/docs/backup-restore) before migrations. The evestack schema
contains durable routines, settings, memory reviews, regression cases and audit
history as well as traces/facts. Use a matching version and a component-specific
repair; do not drop the entire schema to make an older dashboard start.
...
<Warning>
  **Pick a tag that exists.** The examples below use the candidate's `0.5.0` pin.
  A version in this source tree does not prove an image is published. Check the
  registry before changing a running installation:
...
EVESTACK_DASHBOARD_IMAGE=ghcr.io/sammytourani/evestack-dashboard:0.5.0
```
...
<Warning>
  **`"ok":true` is not confirmation that the upgrade landed.** That field answers "is this process
  ...
  ```json
  {"ok":true,"database":"connected","version":"0.5.0"}
  ```
...
Dashboard startup can apply additive schema migrations. Back up the whole
database before upgrading and respect component version guards. Never use
`docker compose down -v` as an upgrade or password-rotation step; it deletes the
database volume. `POSTGRES_PASSWORD` initializes a new volume only. For an existing
database, rotate the role password through an authenticated Postgres session
(for example, interactive `psql`'s `\password`) and update the corresponding private
application configuration together, then restart and verify affected services.
