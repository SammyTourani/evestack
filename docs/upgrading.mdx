...
# whether the four parts are up, and the dashboard's own URL and version
npx evestack status
...
<Warning>
  **Going backward, a newer database now REFUSES an older image, and that is the most visible
  ...
  - `/traces`, `/sessions/[id]`, `/costs`, the overview and `/api/metrics/query` fail; `/sessions`
    degrades to blank fact columns; `/monitors`, `/approvals`, `/schedules`, `/evals` and
    `/charts` still work — the first four because they read only the `workflow` tables, `/charts`
    because it is a static demo. The health response carries the same three lists, so read it
    rather than this paragraph if they ever disagree.
