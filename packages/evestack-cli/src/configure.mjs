...
const PUBLIC_KEYS = new Set([
  ...
  "OLLAMA_BASE_URL",
  "EVESTACK_EMBED_PROVIDER",
  "EVESTACK_EMBED_MODEL",
  "EVESTACK_EMBED_DIMENSIONS",
  "EVESTACK_MEMORY_SCOPE",
...
function readRegular(path, limit = MAX_FILE) {
  ...
  if (!info.isFile() || info.isSymbolicLink())
    throw new Error(
      `Refusing a non-regular configuration file: ${basename(path)}.`,
    );
  if (info.size > limit)
    throw new Error(`Configuration file exceeds ${limit} bytes.`);
  const fd = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW);
  try {
    const bytes = readFileSync(fd);
    if (bytes.length > limit)
      throw new Error("Configuration file grew beyond the size limit.");
    try {
      return new TextDecoder("utf-8", { fatal: true }).decode(bytes);
    } catch {
      throw new Error("Configuration files must be valid UTF-8 text.");
    }
  } finally {
    closeSync(fd);
  }
...
function safeEnv(raw) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (entries.has(key))
      throw new Error(
        `Duplicate ${key} assignments need manual review before configuring.`,
      );
    if (/^["']/.test(value) && !/^(["']).*\1\s*(?:#.*)?$/.test(value))
      throw new Error(
        `Multiline or incomplete quoting in ${key} needs manual review.`,
      );
...
function validateSetting(key, value, allowPublic) {
  ...
  if (!PUBLIC_KEYS.has(key) && !SECRET_KEYS.has(key))
    throw new Error(
      `Unsupported setting ${key}. Edit it in the project's deployment configuration.`,
    );
  ...
  if (
    ...
    throw new Error(
      `Invalid value for ${key}; use a single-line string without control characters or single quotes.`,
    );
  if (/\$(?:[A-Za-z_{])/.test(value))
    throw new Error(
      `Environment interpolation in ${key} needs manual review across the agent and dashboard loaders.`,
    );
  const choices = {
    EVESTACK_PROVIDER: [
      "openai",
  ...
  if (["EVESTACK_BASE_URL", "OLLAMA_BASE_URL"].includes(key)) {
    ...
    if (
      ...
      throw new Error(
        `${key} requires an http(s) URL without embedded credentials.`,
      );
  }
  if (
    /ALLOWED_(USER|GUILD)_IDS$/.test(key) &&
    value
      .split(",")
      .map((v) => v.trim())
      .includes("*") &&
    ...
    throw new Error(
      `${key} opens the channel to everyone. Use --allow-public-channel only if that is intended.`,
    );
}
...
function gitSafety(dir, file) {
  ...
  if (repo.error)
    throw new Error(
      "Could not check Git protection for the configuration file.",
    );
  if (repo.status !== 0) {
    let cursor = dir;
    while (true) {
      if (existsSync(join(cursor, ".git")))
        throw new Error(
          "Git could not inspect this repository. Fix repository access before saving configuration.",
        );
  ...
  if (run(["ls-files", "--error-unmatch", "--", file]).status === 0)
    throw new Error(
      `${file} is tracked by Git. Remove it from tracking and ignore it before saving configuration.`,
    );
  if (run(["check-ignore", "-q", "--", file]).status !== 0)
    throw new Error(
      `Add ${file} to .gitignore before saving configuration so credentials cannot enter a commit.`,
    );
}
...
function protectWindowsPath(path, directory, created = false) {
  ...
  const result = spawnSync(
    ...
    ["-NoProfile", "-NonInteractive", "-Command", script],
    {
      ...
      env: {
        ...process.env,
        EVESTACK_CONFIG_PRIVATE_PATH: path,
        EVESTACK_CONFIG_PRIVATE_DIRECTORY: directory ? "1" : "0",
        EVESTACK_CONFIG_JUST_CREATED: created ? "1" : "0",
      },
    },
  );
  if (result.error || result.status !== 0) {
    const detail = String(
      result.error?.code ?? result.stderr ?? "unknown permission error",
    )
      .replaceAll(path, "[configuration path]")
      .replace(/[\x00-\x1f\x7f]/g, " ")
      .slice(0, 450);
    throw new Error(
      `Could not protect configuration permissions with a Windows owner-only ACL. Nothing was activated. ${detail}`,
    );
  }
}
...
function privateDirectory(path) {
  ...
  if (
    ...
    throw new Error(
      "The backup directory must be owned by you, private (0700), and not a symbolic link.",
    );
...
function atomicWrite(path, text) {
  const temporary = join(
    dirname(path),
    `.${basename(path)}.evestack-${randomUUID()}.tmp`,
  );
  let fd;
  try {
    fd = openSync(
...
export function configurationPlan(
  found,
  changes,
  {
    allowPublic = false,
  ...
  const before = Object.fromEntries(
    ENV_NAMES.map((name) => [name, readRegular(join(dir, name))]),
  );
  ...
  if (restore) {
    ...
    const raw = readRegular(
      join(backupRoot, hash(dir), `${restore}.json`),
      MAX_FILE * 2,
    );
    ...
    if (
      ...
      backup.contentHash !== hash(backup.content)
    )
      throw new Error(
        "The backup does not match this project/file or failed its integrity check.",
      );
    next = backup.content;
    ...
    if (
      ...
      Object.keys(changes).length > 32
    )
      throw new Error(
        "Provide between one and 32 settings as a JSON object or --set options.",
      );
    ...
    for (const [key, value] of Object.entries(changes)) {
      ...
      if (
        ...
        (value === null && parsed.values[key] === undefined)
      )
        continue;
      const index = parsed.entries.get(key);
      const replacement = value === null ? null : `${key}='${value}'`;
      if (index === undefined) {
        if (replacement !== null) parsed.lines.push(replacement);
  ...
  const oldValues = safeEnv(before[file]).values,
    newValues = safeEnv(next).values;
  const keys = [
  ...
  for (const key of keys)
    if (/ALLOWED_(USER|GUILD)_IDS$/.test(key) && newValues[key] !== undefined)
      validateSetting(key, newValues[key], allowPublic);
  ...
  const fingerprint = hash(
    JSON.stringify({ version: 1, dir, before, file, next, restoredFrom }),
  );
        ...
        action:
          oldValues[key] === undefined
            ...
            : newValues[key] === undefined
              ? "remove"
              : "change",
        before: display(key, oldValues[key]),
        after: display(key, newValues[key]),
      })),
      processOverrides: keys.filter(
        (key) => process.env[key] !== undefined && process.env[key] !== "",
      ),
      activation:
        "Not activated. Restart the agent; recreate a dashboard whose environment changed. Then verify and test the relevant provider/channel.",
      notes: [
        ...
        ...(keys.some(
          (key) =>
            key.startsWith("EVESTACK_EMBED_") || key === "EVESTACK_PROVIDER",
        )
          ? [
              "Provider changes can change the embedding model. Back up memory and verify compatibility before activation; changing models requires re-embedding.",
            ]
          : []),
        ...(keys.some((key) => /^(TELEGRAM|SLACK|DISCORD)_/.test(key))
          ? [
              "Channel files, provider-side webhooks and allow-lists must also be configured. Verify an inbound message from an allowed identity; this command sends nothing.",
            ]
          : []),
        ...(keys.some(
          (key) =>
            key.startsWith("EVESTACK_ALERT_") || key === "COMPOSIO_API_KEY",
        )
          ? [
              "Recreate the dashboard to load changed environment values, then check Connections and test receipt.",
            ]
          : []),
...
export function applyConfiguration(plan, expected) {
  if (!expected || expected !== plan.fingerprint)
    throw new Error(
      "The preview fingerprint changed or is missing. Review a fresh preview and pass its --expect value.",
    );
  ...
  try {
    ...
    throw new Error(
      "Another configuration edit may be in progress. Inspect the env lock before retrying.",
    );
  }
  try {
    const readState = () =>
      Object.fromEntries(
        ENV_NAMES.map((name) => [name, readRegular(join(plan.dir, name))]),
      );
    if (hash(JSON.stringify(readState())) !== plan.sourceHash)
      throw new Error(
        "Project configuration changed after preview. Nothing was applied.",
      );
    if (plan.next === plan.before[plan.file])
      return { ...plan.review, applied: false, backupId: null };
    ...
    if (
      ...
      throw new Error(
        "Backups must be outside the project so old credentials cannot be committed.",
      );
    privateDirectory(root);
    const location = join(root, hash(plan.dir));
    ...
    const backup = {
      ...
      project: plan.dir,
      file: plan.file,
      content: plan.before[plan.file],
      contentHash: hash(plan.before[plan.file]),
      createdAt: new Date().toISOString(),
    };
    const backupFd = openSync(
      join(location, `${backupId}.json`),
      constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY,
      0o600,
    );
    try {
      writeFileSync(backupFd, JSON.stringify(backup));
      fsyncSync(backupFd);
    } finally {
      closeSync(backupFd);
    }
    if (hash(JSON.stringify(readState())) !== plan.sourceHash)
      throw new Error(
        "Project configuration changed while backing up. Nothing was applied.",
      );
  ...
  try {
    ...
    for (const arg of argv) {
      if (arg === "--apply") apply = true;
        ...
        throw new Error(
          "Unknown configure option. Run evestack configure --help.",
        );
    }
    if (restore && (fromFile || assignments.length))
      throw new Error("Restore cannot be combined with new settings.");
    const found = findProjectEnv(cwd);
    if (!found) return notAProject(stderr);
    let changes = {};
    if (fromFile) {
      const raw = readRegular(resolve(cwd, fromFile), 65536);
      if (raw === null) throw new Error("Settings file not found.");
      try {
    ...
    for (const assignment of assignments) {
      ...
      if (at < 1) throw new Error("Use --set=KEY=value.");
      const key = assignment.slice(0, at);
      if (SECRET_KEYS.has(key))
        throw new Error(
          `Use --from-file for ${key}; secrets do not belong in shell history.`,
        );
      if (Object.hasOwn(changes, key))
        throw new Error(`Duplicate setting ${key}.`);
      Object.defineProperty(changes, key, {
        value: assignment.slice(at + 1),
        enumerable: true,
        configurable: true,
      });
    }
    const plan = configurationPlan(found, changes, {
      allowPublic,
      restore,
      ...(backupRoot ? { backupRoot } : {}),
    });
    const result = apply
      ? applyConfiguration(plan, expected)
      : { ...plan.review, applied: false };
    if (json) stdout.write(JSON.stringify(result, null, 2) + "\n");
    else {
      stdout.write(`${result.applied ? "Saved" : "Preview"} ${result.file}\n`);
      for (const change of result.changes)
        stdout.write(
          `  ${change.action} ${change.key}: ${change.before ?? "(unset)"} → ${change.after ?? "(unset)"}\n`,
        );
      stdout.write(`Fingerprint: ${result.fingerprint}\n`);
      if (!apply)
        stdout.write(
          "Apply with the same options plus --apply --expect=" +
            result.fingerprint +
            "\n",
        );
      if (result.backupId)
      ...
      if (result.processOverrides.length)
        stdout.write(
          "Shell overrides: " + result.processOverrides.join(", ") + "\n",
        );
      stdout.write(result.activation + "\n");
      for (const note of result.notes) stdout.write(note + "\n");
      stdout.write(
        "Next: evestack verify, then inspect a real task or channel delivery.\n",
      );
    }
    return 0;
  } catch (error) {
    stderr.write(`Configuration was not confirmed: ${error.message}\n`);
    return 1;
  }
}
