...
function readWindow(raw: string | undefined): number {
  ...
  return (OVERVIEW_WINDOWS as readonly number[]).includes(asked)
    ? asked
    : DEFAULT_WINDOW;
}
      ...
      <>
        ...
        <DatabaseError
          error={
            reason ??
            new Error(
              "Every measure on this page failed to read from Postgres.",
            )
          }
        />
      </>
  ...
  return (
    <>
      ...
      <p className="page-sub">
        Every agent run on this machine over the last {label}, read straight
        from your own Postgres. Each number says how much of the window it could
        ...
        <nav aria-label="Time range" className="flex flex-wrap gap-1.5">
          {OVERVIEW_WINDOWS.map((w) => (
            <a
              key={w}
              href={
                w === DEFAULT_WINDOW ? "/overview" : `/overview?window=${w}`
              }
