...
export async function create(argv) {
  ...
  writeFileSync(
    ...
    [
      ...
      "",
      "# Sandbox egress. The agent's shell starts with NO network: deny-all, so",
      "# model-authored code cannot reach your LAN, your router or the internet.",
      "# curl, git clone, pip install, npm install and apt-get install all fail on",
      "# DNS inside it. Uncomment to hand the shell the network back — and then",
      "# start a NEW session, because eve reuses one container per session and",
      "# never re-applies the policy to one that already exists.",
